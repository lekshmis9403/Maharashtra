...
# Define a function to get the top 5 events with maximum rainfall
...
# Define a function to generate the bar graph
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Img(src=app.get_asset_url('Maharashtra_map_station.png'),
                     style={'width': '600px', 'height': '600px', 'display': 'inline-block', 'margin-right': '50px',
                            'margin-bottom': '5px', 'margin-top': '0px'}),
        ], style={'display': 'inline-block'}),
        html.Div([
            html.H1('INDIA METEOROLOGICAL DEPARTMENT', style={'display': 'inline-block', 'vertical-align': 'top', 'margin-top': '10px'}),
            html.H2('Government Of India', style={'text-align': 'center', 'margin-top': '10px'}),
            html.H1('Maharashtra Rainfall', style={'text-align': 'center', 'margin-top': '10px'}),
            html.Img(src=app.get_asset_url('imd finish logo.png'), style={'width': '200px', 'height': '300px'}),
        ], style={'text-align': 'center', 'display': 'inline-block', 'vertical-align': 'top', 'margin-top': '10px'}),
    ...
    dcc.Tabs(id='tabs', value='tab1', children=[
        ...
        dcc.Tab(label='Trendline', value='tab4', style={'backgroundColor': '#BC544B', 'color': 'white'}),
...
def render_content(tab):
    if tab == 'tab1':
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3('Bar-graph Rainfall Distribution of Maharashtra State', style={
                        'text-align': 'center',
                        'display': 'flex',
            ...
            html.Div([
                html.Div([
                    html.H3('Bar-graph Rainfall distribution of Maharashtra Districts', style={
                        'text-align': 'center',
                        'display': 'flex',
                        'justify-content': 'center',
                        'align-items': 'center',
    ...
    html.H4('This is Heatmap Monthly Distribution  of Maharashtra State'),
    ...
    dcc.Graph(id='calendar-heatmap-weekly'),
        ...
        return html.Div([
            html.H3('Extreme Events of Maharashtra State', style={'text-align': 'center'}),
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3('Percentage Distribution Of annual Rainfall in Maharashtra', style={
                        'text-align': 'center',
                        'display': 'flex',
                        'justify-content': 'center',
                        'align-items': 'center',
...
def update_fig(start_date, end_date):
    filtered_data = data[(data['Year'] >= pd.to_datetime(start_date).year) & (data['Year'] <= pd.to_datetime(end_date).year)]
...
# Create the bar graph using Plotly Express with custom color scale and 'Rainfall' as color
    ...
    fig.update_traces(hovertemplate=hover_template, marker_line_color='purple')
...
def update_bar_graph(start_date, end_date):
    fig = update_fig(start_date, end_date)
    return fig
def generate_bar_graph(year, district):
    if pd.isna(year):
    ...
    for station, (_, station_data) in zip(grouped_data.groups.keys(), grouped_data):
        ...
        y_values_flat = [0 if pd.isna(value) else value for value in y_values_flat]
    ...
    hovertemplate = '<b>Station:</b> %{x}<br><b>Date:</b> %{customdata}<br><b>Rainfall:</b> %{y:.2f}'
    fig.update_traces(hovertemplate=hovertemplate,
                      customdata=df['Day'])

    top_events_data = []
    for station, (_, station_data) in zip(df['Station'].unique(), df.groupby('Station')):
        max_rainfall_day = station_data['Day'][station_data['Rainfall'].idxmax(skipna=True)]
        max_rainfall_value = station_data['Rainfall'].max(skipna=True)
    ...
    annotations = []
    max_rainfall_value = df['Rainfall'].max() * 1.02
...
# Callback to update the bar graph and rainfall sum box
@app.callback(
    Output('bar-graph-sum', 'figure'),
    Output('rainfall-sum-box', 'children'),
    [Input('year-dropdown-bar-sum', 'value'),
     Input('district-dropdown-bar-sum', 'value')]
)
def update_graph(year, district):
    fig, rainfall_sum_box_content = generate_bar_graph(year, district)
    return fig, rainfall_sum_box_content
...
def update_statewise_rainfall_graph(year_range):
    ...
    fig = px.bar(df, x='Day', y='Average Rainfall',
                 ...
                 title=f'Maharashtra State Climatology from  - {start_year} to {end_year}',
...
def update_trend_chart(year_range):
    ...
    fig.update_layout(title=f'Station Rainfall Climatology in Maharashtra from ({start_year} to {end_year})',
